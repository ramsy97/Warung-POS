...
import { TrendingUp, Calendar, CreditCard, X, Check } from 'lucide-react';
import api from '../api';
...
const Receivables: React.FC = () => {
  ...
  const [notes, setNotes] = useState('Pelunasan Kredit Piutang');
  ...
  const fetchReceivables = async () => {
    try {
      const res = await api.get('/api/receivables');
  ...
  const handleOpenPay = (r: Receivable) => {
    setActiveReceivable(r);
    setPaymentAmount(r.remaining_amount);
    setNotes(`Pelunasan Cicilan Piutang POS #${r.sale_id}`);
    setErrorMsg('');
    setSuccessMsg('');
  ...
  const handlePaySubmit = async (e: React.FormEvent) => {
    ...
    if (paymentAmount > activeReceivable.remaining_amount) {
      setErrorMsg(`Jumlah pembayaran tidak boleh melebihi sisa piutang (${formatCurrency(activeReceivable.remaining_amount)})`);
      return;
    }

    try {
      await api.post(`/api/receivables/${activeReceivable.id}/pay`, {
        amount: paymentAmount,
        notes
      });
      setSuccessMsg('Pembayaran cicilan piutang berhasil dicatat.');
      setPayModalOpen(false);
      fetchReceivables();
    } catch (err: any) {
      console.error(err);
      setErrorMsg(err.response?.data?.detail || 'Gagal memproses pembayaran piutang.');
    }
  };
  ...
  const calculateTotalUnpaid = () => {
    return receivables.reduce((sum, r) => sum + r.remaining_amount, 0);
  };
  ...
  return (
    <div className="flex-1 p-8 space-y-6 overflow-y-auto max-w-7xl mx-auto w-full transition-colors duration-200">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-800 dark:text-white tracking-wide">Piutang Pelanggan</h1>
          <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Pantau dan Tagih Piutang Belanja Pelanggan</p>
        </div>
        <div className="bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-250 dark:border-emerald-900/40 px-6 py-4 rounded-3xl flex items-center gap-4">
          <TrendingUp className="h-8 w-8 text-emerald-500 shrink-0" />
          <div>
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block">Total Sisa Piutang</span>
            <span className="text-lg font-black text-emerald-600 dark:text-emerald-450">{formatCurrency(calculateTotalUnpaid())}</span>
          </div>
        </div>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-55 dark:bg-slate-850/40 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest border-b border-slate-100 dark:border-slate-800">
              <th className="py-4 px-6">ID POS / PELANGGAN</th>
              <th className="py-4 px-6">TOTAL PIUTANG</th>
            ...
            {receivables.length === 0 ? (
              <tr>
                <td colSpan={6} className="py-12 text-center text-slate-400 font-semibold">
                  Bagus! Tidak ada piutang pelanggan aktif.
                </td>
              </tr>
            ) : (
              receivables.map((r) => {
                const isOverdue = new Date(r.due_date) < new Date() && r.status === 'Belum Lunas';
                return (
                  <tr key={r.id} className="hover:bg-slate-55/30 dark:hover:bg-slate-850/10 transition-colors">
                    <td className="py-4.5 px-6 font-bold">
                      <span className="text-slate-850 dark:text-slate-250 block">POS Transaksi: #{r.sale_id}</span>
                      <span className="text-[10px] text-slate-400 font-semibold uppercase block mt-0.5">Nama: {r.customer?.name || 'Pelanggan'}</span>
                    </td>
                    ...
                    <td className="py-4.5 px-6 font-semibold">
                      <span className={`flex items-center gap-1 ${isOverdue ? 'text-rose-500 font-black' : 'text-slate-500 dark:text-slate-400'}`}>
                        ...
                        {new Date(r.due_date).toLocaleDateString('id-ID', { dateStyle: 'short' })}
                      </span>
                    </td>
                    ...
                    <td className="py-4.5 px-6 text-center">
                      <div className="flex justify-center">
                        {r.status !== 'Lunas' ? (
                          ...
                          >
                            <CreditCard className="h-3 w-3" /> TERIMA PELUNASAN
                          </button>
                        ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 backdrop-blur-sm p-4 animate-fade-in">
          <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-150 dark:border-slate-800 overflow-hidden animate-in scale-in duration-200">
            <div className="px-6 py-5 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-55/50 dark:bg-slate-900/40">
              <h3 className="font-black text-sm text-slate-800 dark:text-white">TERIMA PEMBAYARAN PIUTANG</h3>
            ...
            <form onSubmit={handlePaySubmit} className="p-6 space-y-4">
              <div className="p-4 rounded-2xl bg-slate-55 dark:bg-slate-800/45 text-xs space-y-2 border border-slate-100 dark:border-slate-800">
                <div className="flex justify-between text-slate-500">
                  ...
                  <span className="font-bold text-slate-700 dark:text-slate-300">{activeReceivable.customer?.name}</span>
                </div>
                <div className="flex justify-between text-slate-500">
                  <span>Sisa Tagihan Piutang:</span>
                  <span className="font-extrabold text-slate-850 dark:text-white">{formatCurrency(activeReceivable.remaining_amount)}</span>
                </div>
              </div>
